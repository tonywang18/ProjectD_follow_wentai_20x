...
# image_final, mask_det_cla_final,det_center_dict,image_name
def collate_fn(batch):
    #  batch是一个列表，其中是一个一个的元组，每个元组是dataset中_getitem__的结果
    batch = list(zip(*batch))
    image = batch[0]
    mask_det_cla = batch[1]
...
#  ============================== 定义存储文件路径 =====================================
    ...
    #这里对数据集以及dataloader进行一个定义，注意我在这里由于我们pannuke数据集的图片是40X，而这个模型需要20X，因此rescale=0.5，进行下采样
    #进行
...
# ============================== 进行训练 阶段的切换  ==========================================
    if is_train_from_recent_checkpoint and os.path.isfile(ck_restart_name):
        ...
        #这里加入了一个判断，如果不是在过程控制节点处（下一阶段的第一轮），则去加载本阶段上一轮训练好的模型继续进行训练
        #如果判断刚好在下一阶段的第一轮，那么直接去加载上一阶段最好的模型进行训练
...
#========================================进行训练===========================================
    for epoch in range(start_epoch,epochs):
        ...
        for batch_count in tqdm(range(batch_counts)):
            for index,(image,mask_det_cla_label,det_center_dict,image_name) in enumerate(train_loader):

                # 将6个通道划分为5个通道和1个通道，最后一个通道是检测mask，但是不准确故舍弃
                mask_cla_label, _ = np.split(mask_det_cla_label, [5, ],-1)
                mask_det_label = np.max(mask_cla_label,  -1,keepdims=True)
                ...
                mask_det_label_oh = tr_pm_to_onehot_vec(mask_det_label, soft_label=False)
                mask_cla_label_oh = tr_pm_to_onehot_vec(mask_cla_label, soft_label=True)
                ...
                with torch.no_grad():
                    ...
                    if pg_id > 1:
                        ...
                        cla_acc = ((batch_pred_cla_tmp_bin * mask_cla_label).sum(dim=(0, 2, 3)) / ( mask_cla_label.sum(dim=(0, 2, 3)) + 1e-8)).mean().item()

                        batch_pred_det2_tmp = batch_pred_det2.clamp(0., 1.)
                    else:
                        cla_acc = 0.

                #这里进行了阶段的判断，通过不同的阶段进行loss的计算
                if pg_id == 1:

                    loss = loss_func.det_loss(batch_pred_det.softmax(dim=1), mask_det_label_oh, 'auto')
                elif pg_id == 2:
                    ...
                    loss1 = loss_func.det_loss(batch_pred_det.softmax(dim=1), mask_det_label_oh, 'auto')
                    _tmp = batch_pred_det.softmax(dim=1)[:, 1:2]
        ...
        if (epoch + 1) % 1 == 0:
            with torch.no_grad():
                ...
                for dt in match_distance_thresh_list:
                    det_score_table[dt] = {
                        'found_pred': 0,  # 预测正确的细胞
                        'fakefound_pred': 0,  # 所有的假阳性
                        'found_label': 0,  # 所有找到的标签
                        'nofound_label': 0,  # 所有未找到的标签
                ...
                for valid_index, (valid_image, valid_mask_det_cla_label, valid_det_center_dict, valid_image_name) in enumerate(valid_loader):
...
# ================================== 不知道为什么这里获得的都是元组，所以为了取数据【0】 =======================
                    ...
                    valid_mask_det_cla_label = valid_mask_det_cla_label[0]
# ==================================================================================================================

                    valid_mask_cla_label, _ = np.split(valid_mask_det_cla_label, [5, ], -1)
                    valid_mask_det_label = np.max(valid_mask_cla_label, -1, keepdims=True)
                    plt.imshow(valid_mask_det_label)
                    plt.show()

                    del valid_mask_det_cla_label
...
# ========================================= 开始进行大图的拼接预测 ===============================================
    ...
    # ======================================= mask路径 ========================================
                        #这里定义的是图片以及检测标签以及分类标签图的保存路径
                        eval_image_path = out_dir + str(epoch) + str(valid_image_name) + '_eval_image.png'
                        eval_mask_det_path = out_dir + str(epoch) + str(valid_image_name) + '_eval_mask_det.png'
    ...
    # ======================================= 预测的路径 ========================================
                        ...
                        cv2.imwrite(eval_mask_det_path, valid_mask_det_label * 255)
...
if __name__ == '__main__':
    from model.model_1_Pannuke_wenTai import MainNet
    main(MainNet)