...
# -*- coding: UTF-8 -*-
'''
@Project ：ProjectD_follow_wentai_20x 
...
class DatasetReader:
    ...
    def __init__(self, dataset_path=os.path.join('F:\生物图腾公司的数据集和代码\pannuke_packed_dataset'), data_type='fold_1',
                 pt_radius_min=3, pt_radius_max=3,transforms=None,rescale=1,use_repel=False):
...
# ======================================== 通过中心点 获得 mask ==========================================
            ...
            label = np.zeros([im.shape[0], im.shape[1], self.class_num], np.float32)


# ===============  根据标签mask的坐标中心，画出 mask  ================================================
            for k in label_cls_center_pts.keys():
                    ...
                    if not self.use_repel:
                        ...
                        draw_repel_code_fast(label[:, :, k:k + 1], pts, self.pt_radius_max, A=0.5)


            label_center = copy.deepcopy(label_cls_center_pts)

            self.image_results.append(im)
            self.mask_label_results.append(label)
    ...
    def __getitem__(self, item):  # 返回数据和标签
        ...
        mask_label = self.mask_label_results[item]
        label_center_dict = self.label_center_dict[item]
        image_name = self.image_name_results[item]

        img_mask_enhance = self.transform(image=image, mask=mask_label)     # 对检测mask和分类mask同image进行相同的变换操作
        image_final = img_mask_enhance['image']                                      # 获得数据增强后的 image
        mask_label = img_mask_enhance['mask']                                # 获得数据增强后的 mask标签

        # image_final = np.transpose(image_final / 255.0, (2, 0, 1)).astype(np.float32)  # 将数据进行归一化，并且交换通道顺序，和tensor的类型
        # mask_det_cla_final = np.transpose(mask_det_cla, (2, 0, 1)).astype(np.float32)

        return image_final, mask_label,label_center_dict,image_name
...
if __name__ == '__main__':
    ...
    train_dataset = DatasetReader(r'F:\生物图腾公司的数据集和代码\pannuke_packed_dataset\\','fold_1',
                                  pt_radius_min=9, pt_radius_max=9,transforms=transform,use_repel=True,rescale=0.5)
    train_loader = DataLoader(dataset=train_dataset, batch_size=1, shuffle=True, num_workers=0,collate_fn=collate_fn)
    for epoch in range(100):
        for index,(image, mask_det_cla_final,det_center_dict,image_name) in enumerate(train_loader):

            image = image[0][:, :, :]
            mask_det_cla = mask_det_cla_final[0][:, :, :]
            det_center_dict = mask_det_cla_final[0]

            image_name = image_name[0]

            cv2.imshow('image', image)
            cv2.imshow('mask_cla_1', mask_det_cla[:,:,1])
            cv2.imshow('mask_cla_0', mask_det_cla[:, :, 0])
            cv2.waitKey(0)
